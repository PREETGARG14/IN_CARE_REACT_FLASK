...
from market.models import Patient,AdminUser, past_history_of_illness
from market.forms import RegisterForm, LoginForm,PurchaseItemForm,SellItemForm,AdminLoginForm, AdminAddPatientForm
...
def edit_patient_page(page_id):
    ...
    if form.errors != {}: #If there are not errors from the validations
        for err_msg in form.errors.values():
            flash(f'There was an error with creating a user: {err_msg}', category='danger')
    return render_template('edit_details.html', past_history = past_history)
